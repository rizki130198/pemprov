...
class GroupController extends Controller
{
    ...
    public function group($id){
        ...
        if ($validasi) {
            if (request()->segment(2) == "diskusi") {
                return view('groups.group', compact('id_link','user' ,'group', 'wall','user_list','groups','anggota','admin','myuser','images_grup'));
    ...
    public function deleteMemberGrup($id,$idgrup)
    {
        $response = array();
        $response['code'] = 400;
        if (Auth::user()->id != $id) {
            $update = User_grup::where('id_user',$id)->where('id_groups',$idgrup)->delete();
            $response['code'] = 200;
        }else{
            $response['code'] = 400;
    ...
    public function leaveGrup($id,$idgrup)
    {
        $response = array();
        $response['code'] = 400;
        if (Auth::user()->id == $id) {
    ...
    public function deleteAdminGrup($id,$idgrup)
    {
        $response = array();
        $response['code'] = 400;

        if (Auth::user()->id != $id) {
            $update = User_grup::where('id_user',$id)->where('id_groups',$idgrup)->get()->first();
            $update->jabatan_grup = 'member';
    ...
    public function addAdmin($id,$idgrup)
    {
